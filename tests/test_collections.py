...
@pytest.mark.asyncio
async def test_collections_resync(pb_client):
    PBModel.bind_client(pb_client)
    await Addon.sync_collection()
    time.sleep(
        1
    )  # we need one second sleep as filenames from pocketbase have second timestamps
    await Addon1.sync_collection()
    time.sleep(1)
    await Addon2.sync_collection()
    time.sleep(1)
    # resync collections
    await Addon2.sync_collection()
    time.sleep(1)
    await Addon.sync_collection()
    time.sleep(1)
    await Addon2.delete_collection()
    time.sleep(1)
    await Addon2.sync_collection()
    time.sleep(1)
    ...
    await Addon2Removed.sync_collection()
    time.sleep(1)
    await Addon2Removed.sync_collection()
